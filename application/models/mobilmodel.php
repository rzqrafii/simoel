...
class Mobilmodel extends CI_Model {
    ...
    function GetData($id) {
    	...
    	$id = $this->uri->segment(3);
    	return $this->db->get_where('kendaraan', array('idKendaraan'=> $id))->row();
    }

    function getLastId()
	{
		return $this->db->select('MAX(idKendaraan) as id')
		->get('kendaraan')
		->row()->id;
	}
    ...
    /*public function getMerk(){
  		...
  		$this->db->from("kendaraan");
		$this->db->order_by("merkKendaraan", "ASC");
		$query = $this->db->get(); 
		return $query->result();
 	...
 	/*public function insert()
	{
		...
		return $this->db->insert('kendaraan', $input);
	}*/

	public function delete($param) {
		return $this->db->delete('kendaraan', array('idKendaraan' => $param));
	}
